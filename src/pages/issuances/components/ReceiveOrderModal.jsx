...
const ReceiveOrderModal = (props, ref) => {
	...
	const updateIssuedQty = (id, issuedQty, defaultQty) => {
		setData((prevData) => ({
			...prevData,
			details: prevData?.details.map((prevDetail) => ({
				...prevDetail,
				items: prevDetail?.items?.map((x) =>
					x.id == id ? { ...x, received_qty: issuedQty } : x
				),
			})),
		}));
	};
	...
	const submitIssuanceForm = () => {
		...
		data?.details?.map((detail) => {
			...
			detail?.items.map((item) => {
				formData.append(
					`received_qty[${detail?.id}][${item?.id}]`,
					item?.received_qty
				);
			});
		});
		axios
			.post(`/inventory/issuances-recieved/${data?.id}`, formData)
			.then((res) => {
				setLoading(false);
				setTimeout(() => {
	...
	return (
		<Modal open={open} hide={hide} size="xl">
			<ModalHeader title={"Receive order form"} hide={hide} />
...
export default forwardRef(ReceiveOrderModal);
